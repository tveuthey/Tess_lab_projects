...
for i = 1:length(Blocks)
    ...
    for chan_no=1:8
         ...
         for i = 1%:7
            ...
            N{chan_no,i+1} = TTX.ReadEventsV(eventnumber, 'eTt1', chan_no, 0, 0.0, 0.0, 'FILTERED');
            disp([chan_no])
            disp([])
    ...
    for chan_no = 1:32
        ...
        tmpspkdata=TTX.ReadWavesV('pTt1');
...
%     
    ...
    %saving raw data
    outdir = 'C:\Users\dhaks_000\Documents\MATLAB\Dhakshin\Mat_files\raw_data\Reach\T39\';
    if (~exist(outdir))
        mkdir(outdir);
    end
    cd (outdir)
