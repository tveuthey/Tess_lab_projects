...
%     if max(PEH_rate)>max(PEH_rate2) & y_max < max(PEH_rate)  % finds the maximum rate for all targets
    ...
    hold off
     titlename=['Block 310 Reach ON SC2 Ch ' num2str(chan_no)];
     title(titlename)
%
    ...
    plot ([(t(1):0.050:t(end))-t(1)],1*PEH_rate/y_max,'r','LineWidth',2);XLim([0 max_ax2]);YLim([0 1]);set(gca,'XTick',[0 f2 f2*2 f2*3 f2*4 f2*5]);set(gca,'XTickLabel',{'-4','-3','-2','-1','Reach ON','1'})
    hold on;
    plot ([(t(1):0.050:t(end))-t(1)],1*PEH_rate2/y_max2,'k','LineWidth',2);XLim([0 max_ax2]);YLim([0 1]);set(gca,'XTick',[0 f2 f2*2 f2*3 f2*4 f2*5]);set(gca,'XTickLabel',{'-4','-3','-2','-1','Reach ON','1'});set(ax4, 'XGrid', 'on');set(ax4, 'YGrid', 'on');
    hold off;
    ...
    for l_incorr=1:length(incorr)
        ...
        subplot(5,1,4)
        plot(rastertics5{l_incorr},[length(corr) length(corr)+1]+l_incorr,'k','LineWidth',2);plot(doordelta_incorr(l_incorr),[length(corr) length(corr)+1]+l_incorr,'cx');XLim([0 max_ax2]);YLim([0 max_trials]);set(gca,'XTick',[0 1 2 3 4 5]);set(gca,'XTickLabel',{'-4','-3','-2','-1','Reach ON','1'}) %axis([0 2.4 (0+n4) (1+n4)]);%hold off;
        hold on
    end
    %xlabel('time (sec)')
    %     subplot(4,1,4)
    ...
    hold on;
    plot(-mean_hold_x_incorr,'k-','LineWidth',2);XLim([(c3-120) (c3+36)]);set(ax3, 'XGrid', 'on');set(gca,'XTick',[c3-120  c3-90 c3-60 c3-30 c3 c3+30]);set(gca,'XTickLabel',{'-4','-3','-2','-1','Reach ON','1'}); xlabel ('time (sec)')

    hold off;
    
        filename=['Mat_files/Result_figs/Block10_SC2_ReachON_Chan' num2str(chan_no) '.tiff'];
