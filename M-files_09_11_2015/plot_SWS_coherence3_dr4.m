function [Change_sws_med Change_spindle_med]=plot_SWS_coherence3_dr3(TimeStamps1,TimeStamps2,data1,data2, time_pre1,time_post1,units, chan_no, sc, bin_size, space, Fs_lfp, taper, win,dir)
figure
close all
...
bin_vector=[round(time_pre(1)/Fs_lfp):bin_size:round(time_pre(end)/Fs_lfp)];
...
for j = 1:length(time_post1)    
    ...
    if(numel(ts2atmp)>0)
        ts2atmp2=(ts2atmp-ts2atmp(1))+1;
        if numel(ts2a)>0
            ts2atmp2=ts2atmp2+ts2a(end);
            ts2a = [ts2a ts2atmp2];
        else ts2a = [ts2atmp2];
        end
    end
...
%         %     lfp_pre=lfp_pre;
        ...
        %     plot(f_post,C_post,'r');
        pltlng = length(find(f_pre<20));
        h = figure;shadedErrorBar(f_pre(1:pltlng),smooth(C_pre(1:pltlng),20),cat(2,smooth(Cerr_pre(1,1:pltlng),20),smooth(Cerr_pre(2,1:pltlng),20))','b',1);
        hold on;
        shadedErrorBar(f_post(1:pltlng),smooth(C_post(1:pltlng),20),cat(2,smooth(Cerr_post(1,1:pltlng),20),smooth(Cerr_post(2,1:pltlng),20)),'r',1);
        ...
        filename = strcat(dir,'SFC post_FP-SFCloc', num2str(sc), '_Ch', num2str(unit),'.tiff');
        saveas(h,filename);
        close all;
...
%         
    ...
    cd(dir)
    savefile = strcat('coherdataloc_SC_', num2str(sc), '.mat');
    ...
    'Change_sws_med', 'Change_spindle_med', 'sws_premax_med', 'sws_postmax_med', 'sws_premean_med', 'sws_postmean_med', 'spindle_premax_med', 'spindle_postmax_med', 'spindle_premean_med', 'spindle_postmean_med', 'phi_pre_sws_med', 'phi_pre_spindle_med', 'phi_post_sws_med', 'phi_post_spindle_med', 'phi_change_sws_med' ,'phi_change_spindle_med');
