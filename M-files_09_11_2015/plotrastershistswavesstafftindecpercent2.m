function [Output]=plotrastershistswavesstafftindecpercent2(TimeStamps, chan_no,data,wave,Fs_lfp,Waves,N,sc,bin_size,space)

close all;
clear c1 c2 c3 chan_inc chan_dec chan_unchanged not eval c_empt per_inc per_dec
c1=[0];
c2=[0];
c3=[0];
chan_inc=[];
            ...
            if N{chan_no(i),sc(k)}<=100 && N{chan_no(i),sc(k)}>=10
                ...
                if P<0.05
                    if mean(dNpre)<mean(dNpost)
                        ...
                        per_inc=((max(dNpost)-mean(dNpre))/mean(dNpre))*100;
                        if k==1
                            chan_inc=[chan_inc; i];
                            per_inc=[i, per_inc];
                        else
                            if k==2
                                ...
                                per_inc=[i+32, per_inc];
                            else
                                if k==3
                                    chan_inc=[chan_inc; i+64];
                                    per_inc=[i+64, per_inc];
                                else
                                    if k==4
                                        chan_inc=[chan_inc; i+96];
                                        per_inc=[i+96; per_inc];
                                    end
                                end
                            end
                        end
                        ...
                        per_dec=abs(((mean(dNpre)-min(dNpost))/mean(dNpre))*100);
                        if k==1
                            chan_dec=[chan_dec; i];
                            per_dec=[per_dec; per_dec];
                        else
                            if k==2
...
Output.Total_Cells= [Total_Cells Cells_Modulated Cells_Modulated_Percentage ]
Output.Cells_Modulated=[Cells_Inc Cells_Inc_Percentage  Cells_Dec Cells_Dec_Percentage  Cells_Unchanged Cells_Unchanged_Percentage]
