function [Change_sws_med Change_spindle_med]=plot_reachtask_spkfld_coh(TimeStamps1,data1,pelletdrop,units, sc, bin_size,Fs_lfp, taper, dir)
figure
close all
        params.Fs=Fs_lfp;
        params.fpass=[0 150];
...
trl_num = length(pelletdrop);
trl=zeros(trl_num,trl_len);
trl_data=zeros(trl_len,trl_num);
for i = 1:length(pelletdrop)
trl(i,:) = round(pelletdrop(i)-trl_len:pelletdrop(i)-1);
trl_data(:,i) = med_data(trl(i,:));
end

% bin_vector=[round(1/Fs_lfp):bin_size:round(Fs_lfp)/Fs_lfp)];
...
for i=1:length(units)
   ...
   %[C_pre,phi_pre,S12_pre,S1_pre,S2_pre,f_pre,zerosp_pre,confC_pre,phistd_pre,Cerr_pre]=coherencycpb(trl_data,dN_pre,win,params,1);
   
  [C,phi,S12,S1,S2,f,zerosp,confC,phistd,Cerr]=coherencycpb(trl_datatmp,dN_pre,params,0);
 %  [C2,phi,S12,S1,S2,f,zerosp,confC,phistd,Cerr]=coherencycpb(trl_datatmp,dN_pre,params,0);
  % scale=2;
    so = find((f>1 & f<4));
    th = find((f>4 & f<8));
    al = find((f>8 & f<12));
   ...
   imagesc(C, [0 1]); colorbar;
    ...
    %Cswpre = mean(C(,1:cohnum),1);
    %Cswpost = mean(C((1:8)/scale,cohnum:end),1);
