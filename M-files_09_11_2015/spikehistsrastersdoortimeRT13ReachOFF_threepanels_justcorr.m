...
for chan_no_forchew=[chan_no_forchew]
        ...
        hold on;
        hold_val(n)=mean(sm_power(47:60));      %<=====change this window if classifier is not looking in correct areas of power difference
        plot(2.5,mean(sm_power(47:60)),'rx');
...
%     hold off;
    ...
    max_trials=length(corr);%trials_NO
    max_ax2=length(bin_vector)/20;
    f2=1;
    
    plot ([(t(1):0.05:t(end))-t(1)],1*PEH_rate/y_max,'r','LineWidth',3);XLim([0 max_ax2]);YLim([0 1]);set(gca,'XTick',[0 f2 f2*2 f2*3 ]);set(gca,'XTickLabel',{'-2','-1','Reach OFF','1'},'FontSize',14,'FontWeight','BOLD');set(gca,'YTick',[0 0.5 1 ]);set(gca,'YTickLabel',{'0','','1'},'FontSize',14,'FontWeight','BOLD');set(ax4, 'XGrid', 'on');set(ax4, 'YGrid', 'on');
    hold on;
%     plot
%     ([(t(1):0.05:t(end))-t(1)],1*PEH_rate2/y_max2,'k','LineWidth',3);XLim([0 max_ax2]);YLim([0 1]);set(gca,'XTick',[0 f2 f2*2 f2*3 ]);set(gca,'XTickLabel',{'-2','-1','Reach ON','1'},'FontSize',14,'FontWeight','BOLD');set(gca,'YTick',[0 0.5 1 ]);set(gca,'YTickLabel',{'0','','1'},'FontSize',14,'FontWeight','BOLD');set(ax4, 'XGrid', 'on');set(ax4, 'YGrid', 'on');
    hold off;
         titlename=['Block 381 Reach OFF SC2 Ch ' num2str(chan_no)];
    ...
    for lcorr=1:length(corr)
        ...
        subplot(3,1,2)
        plot(rastertics3{lcorr},[0 1]+lcorr,'r','LineWidth',2);plot(doordelta(lcorr),[0 1]+lcorr,'gx');XLim([0 max_ax2]);YLim([0 max_trials]);set(gca,'XTick',[0 1 2 3 ]);set(gca,'XTickLabel',{'-2','-1','Reach OFF','1'},'FontSize',14,'FontWeight','BOLD'); %set(gca,'YTickLabel','FontSize',14,'FontWeight','BOLD')%axis([0 2.4 (0+n4) (1+n4)]);%hold off;
        hold on
    end
%     hold on
...
%     hold on;
    plot(t,(-mean_hold_x_corr),'r-','LineWidth',3);XLim([(c3-60) (c3+36)]);YLim([(min(-mean_hold_x_corr)-5) (max(-mean_hold_x_corr)+5)]);set(gca,'XTick',[c3-60 c3-30 c3 c3+36]);set(gca,'XTickLabel',{'-2','-1','Reach OFF','1'}); set(gca,'YTick',[(min(-mean_hold_x_corr)-5) (((min(-mean_hold_x_corr)-5)+(max(-mean_hold_x_corr)+5))/2) (max(-mean_hold_x_corr)+5)]); set(gca,'YTickLabel',{'Initial','','Target'},'FontSize',14,'FontWeight','BOLD');xlabel ('time (sec)','FontSize',16,'FontWeight','BOLD');set(ax9, 'XGrid', 'on');set(ax9, 'YGrid', 'on');

    hold off;
%     
        filename=['Mat_files/Result_figs/Block381_SC2_ReachOFF_SELECTDATACORRChan' num2str(chan_no) '.tiff'];
