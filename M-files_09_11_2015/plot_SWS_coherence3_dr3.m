...
nyq_sample=srate/2;
        ...
        sws_pre=(filtfilt(b2,a2,lfp_pre));
        sws_post=(filtfilt(b2,a2,lfp_post));

            FILTER_ORDER=5;
            f_lo=18;
            f_hi=8;
            [b2,a2]=butter(FILTER_ORDER,[f_hi f_lo]/nyq_sample);
            spindle_pre=(filtfilt(b2,a2,lfp_pre));
            spindle_post=(filtfilt(b2,a2,lfp_post));
...
%    ts2a = ts2tmp(find(ts2tmp>time_post(1) & ts2tmp<time_post(end)));
    ...
    if((numel(ts1)>250) & numel(ts2>250))
        ...
        filename = strcat(dir,'cens\SFC post_FP-SC_', num2str(sc), '_Ch', num2str(unit),'.tiff');
        saveas(h,filename);
        close all;
