function [Output]=plotrastershistswaves(TimeStamps, chan_no,data,wave,Fs_lfp,Waves,N,sc,bin_size,space)
close all;
...
for i=1:length(chan_no)
    ...
    if N{chan_no(i),sc}<=500
        ...
        subplot(3,1,2);
        plot((+1)*temp3,'k', 'LineWidth',3); XLim([0 length(temp3)]); YLim([-0.2 1.2]);title('Anodal tDCS Stimulation','FontSize',16,'FontWeight','BOLD')
%         plot((-1)*temp3,'k', 'LineWidth',3); XLim([0 length(temp3)]); YLim([-1.2 0.2]);title('Cathodal tDCS Stimulation','FontSize',16,'FontWeight','BOLD')
        ...
        set(gca,'YTick',[0 1],'YTickLabel',{'-0.2','1.2'},'FontSize',14,'FontWeight','BOLD')
        ...
        subplot(3,4,[9 10 11 12]); plot([rastertics{i};rastertics{i}],[0 Yax_lim],'g','Linewidth',1); XLim([0 (length(data)/Fs_lfp)]); hold on %hold on; plot([(temp3)/Fs_lfp],1,'r'); hold off;
        ...
        % for i=1:length(chan_no)
        subplot(3,4,1)
        plot(waves,'g');XLim([0 30]);YLim([-0.00015 0.00015]); set(gca,'XTick',[0 30],'FontSize',14,'FontWeight','BOLD');set(gca,'XTickLabel',{'0','1.2'},'FontSize',14,'FontWeight','BOLD'); set(gca,'YTick',[-0.00015 0 0.00015],'FontSize',14,'FontWeight','BOLD');set(gca,'YTickLabel',{'-150','0','150'},'FontSize',14,'FontWeight','BOLD'); xlabel('t(msec)','FontSize',14,'FontWeight','BOLD'); ylabel('microvolt','FontSize',14,'FontWeight','BOLD'); %title('Unit 1','FontSize',14,'FontWeight','BOLD')
        subplot(3,4,[2 3 4])
        bar(histc(diff(tstamps2),[0.005:0.001:0.100]),'g','Linewidth',3);set(gca,'XTick',[0 20 40 60 80 100],'FontSize',14,'FontWeight','BOLD');set(gca,'XLim',[0 40.5]);set(gca,'XTickLabel',{'0.00','0.02','0.04','0.06','0.08','0.10'},'FontSize',14,'FontWeight','BOLD');xlabel('Interspike Interval (s)','FontSize',14,'FontWeight','BOLD'); ylabel ('Counts per bin','FontSize',14,'FontWeight','BOLD'); title('R37 +50 microA 15 min Block 260 SC3','FontSize',14,'FontWeight','BOLD')
        filename=['Mat_files/Result_figs/New/R37 +50 microA 15 min Block 260 SC3 Chan' num2str(i) '.tiff'];
        saveas(gcf,filename);
    end
end
