function generate_PETH_SL(path_to_data, save_path, reach_block, wave_samp_freq, PETH_size, PETH_bin, event)
...
[correct_beamblock_starts,beamblock_starts, pelletdrop_markers]=find_pulses_reach_M1M2_TV_20150925(Reach_mat_wave);
...
colormap jet;
hold on;
line([(((abs(PETH_size(1)) + abs(PETH_size(2))) / PETH_bin)*2/3),(((abs(PETH_size(1)) + abs(PETH_size(2))) / PETH_bin)*2/3)],[1,size(heat_plot1,1)],'Color','r','LineWidth',2,'linestyle',':')
hold off
...
colormap jet;
hold on;
line([(((abs(PETH_size(1)) + abs(PETH_size(2))) / PETH_bin)*2/3),(((abs(PETH_size(1)) + abs(PETH_size(2))) / PETH_bin)*2/3)],[1,size(heat_plot2,1)],'Color','r','LineWidth',2,'linestyle',':')
hold off
