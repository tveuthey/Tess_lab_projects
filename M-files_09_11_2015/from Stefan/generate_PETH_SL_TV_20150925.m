...
% get pelletdrop_markers in ms
[~, pelletdrop_markers]=find_pulses_reach_M1M2_TV_20150925(Reach_mat_wave);
    ...
    for num_units = 1:size(Reach_mat,2)
        if ~isnan(Reach_mat{num_channels,num_units})
            ...
            for trial_num = 1:length(pelletdrop_markers)
                ...
                temp_unit = temp_unit(temp_unit > (pelletdrop_markers(trial_num)+PETH_size(1)) & temp_unit < (pelletdrop_markers(trial_num)+PETH_size(2))); %take chunk around reach (-4 to +2s)
...
save(sprintf('PETH_mat_bin=%d.mat',PETH_bin), 'PETH_mat', 'index', 'num_real_units','PETH_bin')
...
% plot all units (z-scored)
fig = figure;
imagesc(heat_plot)
colorbar;
colormap jet;
hold on;
line([(((abs(PETH_size(1)) + abs(PETH_size(2))) / PETH_bin)*2/3),(((abs(PETH_size(1)) + abs(PETH_size(2))) / PETH_bin)*2/3)],[1,size(heat_plot,1)],'Color','r','LineWidth',2,'linestyle',':')
hold off
saveas(fig,sprintf('raw_all_unit_PETH_bin=%d',PETH_bin),'tiff')

%% z-scored data
clear heat_plot count
heat_plot = zeros(num_real_units,floor(((abs(PETH_size(1)) + abs(PETH_size(2))) / PETH_bin)));
count = 1;
...
% hold off
% saveas(fig,sprintf('zscored_all_unit_PETH_bin=%d',PETH_bin),'tiff')
