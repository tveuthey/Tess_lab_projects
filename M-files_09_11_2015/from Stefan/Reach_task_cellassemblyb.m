function [Activities, Patterns, eigenvalues, lambda_max]=Reach_task_cellassemblyb(TimeStamps,trial_marker,Fs_lfp,bin,units,percblock,len_bef,len_aft,markertype)
...
%         else
%             Task_bin_all(count_all,:)=zeros(1,length(bin2));
        end
        
        if ~isempty(dN_cell_end)
            Task_bin_end(count_all,:) = dN_cell_end;
%         else
%             Task_bin_end(count_all,:)=zeros(1,length((bin2)));
        end
    end
end
