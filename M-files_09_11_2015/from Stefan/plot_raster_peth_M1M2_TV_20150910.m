...
function plot_raster_peth_M1M2_TV(units,trial_marker,TimeStamps,len_bef,len_aft,bin,dir,type)
...
for a = 1:length(units)
    ...
    for i = 1:length(units_sc)
        ...
        if (max(max(peth(count).bin))>1)
            ...
            plot(zeros(int8(yl)+1,1),[0:1:yl],'--k');
            mo = [type,' onset'];
            ...
            t2 = strcat(dir,'PETH Unit',num2str(units_sc(i)),'_SC',num2str(sc),'_Centered_to_',type,'_reachtrials');            
...
if exist('tm')
    ...
    t2 = strcat(dir,'_Spike_Timing_Positive_TR',type,'reachtrials'); %previous title line    
    %pause;
    saveas(gcf,t2,'tiff');
end
close all;
...
if exist('tm')
    ...
    t2 = strcat(dir,'_Spike Timing Negative TR',type,'reachtrials');
    saveas(gcf,t2,'tiff');
    close all;
end
%pause;
savefile = [dir,'_peth2_reachtrialsonly_centeredto_',type,'.mat'];
