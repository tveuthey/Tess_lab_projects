function [Output]=plot_learning_PSTHs_cent_reward_BMI2(TimeStamps, units,data,wave,start_trial, start_pulse2, Fs_lfp,Waves,N,sc,bin_size,space)
close all;
...
for i=1:length(units)
    ...
    for k=size(bin_rate_s,2);
        k1=round(k/3);
    end
    
    %eliminate next four lines for non-article
    PEH_rate4=mean(bin_rate_s(:,:),2);
    max_PEH=max(PEH_rate4);
    PEH_rate4=PEH_rate4/max_PEH;
    plot (bin_vector,smooth(PEH_rate4,10),'b','LineWidth',2);
    
    %eliminate up four lines for non-article regular analysis
    
    % PEH_rate1=sum(bin_rate_s,2)/(total_trials)/0.05
    PEH_rate1=mean(bin_rate_s(:,1:(k1-1)),2);
    PEH_rate2=mean(bin_rate_s(:,(k1:((2*k1)-1))),2);
    PEH_rate3=mean(bin_rate_s(:,((2*k1):((3*k1)-1))),2);
...
%     figure
    ...
    if sc==2
        filename=['C:\Users\Ganguly Lab\Documents\MATLAB\Tanuj\Mat_files\Result_figs\BMI\S23\2_26_13\Task Every Third Trial PSTH_cent_reward Ch' num2str(chan_no) '.tiff'];
        set(gcf, 'Position', get(0,'Screensize'));
%         saveas(gcf,filename);
    else
        if sc==3
            filename=['C:\Users\Ganguly Lab\Documents\MATLAB\Tanuj\Mat_files\Result_figs\BMI\S23\2_26_13\Task Every Third Trial PSTH_cent_reward SC2 Ch' num2str(chan_no) '.tiff'];
            set(gcf, 'Position', get(0,'Screensize'));
%             saveas(gcf,filename);
        end
    end
