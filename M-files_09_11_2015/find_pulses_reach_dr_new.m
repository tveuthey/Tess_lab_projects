...
function [trials_markers, Reach, Reward, noReward]=find_pulses_reach_dr_new(wave,data,Succ,Fail)

%trial structure is: trial onset(5); trial number (+1+1); door open (4),
%audio cue (2) (3)
%trial number then 5(trial onset) 2 (door open) 4 (audio cue),
%3(pellet drop),
wave=wave(1,:);
temp3=wave>.05;
% plot(temp3)
...
for n = 1:length(find_gaps)
    if (n<3)
        if(find_gaps(n)==5)
            ...
            trial_no= [trial_no; find_gaps(n+1)-1, find_gaps(n+2)-1];
%             if (find_gaps(n-1)==3 || find_gaps(n-1)==4)
...
%             end
        ...
        if (find_gaps(n)==5 &&~(find_gaps(n-1)==5 || find_gaps(n-2)==5))
            ...
            if (find_gaps(n-1)==3 || find_gaps(n-1)==4)
                 three_pulses = [three_pulses n];
                 att_trial_no=[att_trial_no;find_gaps(n+1)-1, find_gaps(n+2)-1];
            else three_pulses = [three_pulses 0];
            end
       
        end
    end
end
att_trial = (10*att_trial_no(:,1)+att_trial_no(:,2))-1;
all_att = three_pulses(three_pulses~=0);
succ_att = three_pulses(Succ(1:length(three_pulses)==1));
fail_att = three_pulses(Fail(1:length(three_pulses)==1));
fail_att(fail_att~=0);
succ_att(succ_att~=0);
all_att = sort([fail_att;succ_att]);
Reach = all_att;
Reward = succ_att;
noReward = fail_att;
trials_markers = five_pulses;
