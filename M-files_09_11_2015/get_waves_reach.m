function [trials_markers]=get_waves_reach(wave)
...
for n=2:length(four_pulses)
    if (four_pulses(n)- four_pulses(n-1)) == thresh2
        r_fours=[r_fours n-1];
    end
end
r_four_pulses_time=four_pulses_time(r_fours);

%have a marker for two pulses 
r_twos=[];
thresh3=2;
for n=2:length(two_pulses)
    if (two_pulses(n)- two_pulses(n-1)) > thresh3 %&& (two_pulses(n)- five_pulses(n)) > thresh3
        r_twos=[r_twos n-1];
    end
end
