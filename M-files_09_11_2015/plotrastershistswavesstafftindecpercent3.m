function [Output]=plotrastershistswavesstafftindecpercent3(TimeStamps, chan_no,data,wave,Fs_lfp,Waves,N,sc,bin_size,space)

close all;
clear c1 c2 c3 chan_inc chan_dec chan_unchanged not eval c_empt per_inc per_dec dur_inc dur_dec
cd ('C:\Users\Ganguly Lab\Documents\MATLAB')
c1=[0];
c2=[0];
c3=[0];
chan_inc=[];
...
for k=1:length(sc);
    ...
    for i=1:length(chan_no);
        ...
        if N{chan_no(i),sc(k)}<=10
            ...
            if N{chan_no(i),sc(k)}<=100 && N{chan_no(i),sc(k)}>=10
                disp(i)
                ...
                if k==1
                    ...
                    if k==2
                        not_eval=[not_eval; i+32];
                    else
                        if k==3
                            not_eval=[not_eval; i+64];
                        else
                            if k==4
                                not_eval=[not_eval; i+96];
                            end
                        end
                    end
                end
                ...
                tstamps1=TimeStamps{chan_no(i),sc(k)};
                
                tstamps_ind=find(tstamps1<(round(length(data)/Fs_lfp)));
                tstamps=tstamps1(tstamps_ind);
                ...
                if P<0.05
                    if mean(dNpre)<mean(dNpost)
                        ...
                        per_inc2=((max(dNpost)-mean(dNpre))/mean(dNpre))*100;
                        if mean(dNpost)==0
                            dur_inc2=start2/Fs_lfp;
                        else
                            dur_inc_ind=find((dNpost<(mean(dNpost)+(4.5*std(dNpost)))),1,'last');
                            dur_inc2=tpost(dur_inc_ind(1));
                            if k==1
                                chan_inc=[chan_inc; i];
                                per_inc=[per_inc;per_inc2];
                                dur_inc=[dur_inc;dur_inc2];
                            else
                                if k==2
                        ...
                        per_dec2=((mean(dNpre)-min(dNpost))/mean(dNpre))*100;
                        if mean(dNpost)==0
                            dur_dec2=start2/Fs_lfp;
                        else
                            dur_dec_ind=find((dNpost<(mean(dNpost)+(4.5*std(dNpost)))),1,'last');
                            dur_dec2=tpost(dur_dec_ind(1));
                            if k==1
                                chan_dec=[chan_dec; i];
                                per_dec=[per_dec; per_dec2];
                                dur_dec=[dur_dec;dur_dec2];
                            else
                                if k==2
                ...
                for jpost=5:length(tstamps_post)-30
                ...
                % cd ('C:\Users\Ganguly Lab\Documents\MATLAB')
                if k==1
                    filename=['Tanuj/Mat_files/Result_figs/New (Acute_Neuromod_I)/T1/Cat_Block SC2 Chan' num2str(chan_no(i)) '.tiff'];%C:\Users\Ganguly Lab\Documents\MATLAB\Tanuj\Mat_files\Result_figs\New (Acute_Neuromod_I)\S33
                    saveas(gcf,filename);
                else
                    if k==2
                        filename=['Tanuj/Mat_files/Result_figs/New (Acute_Neuromod_I)/T1/Cat_Block SC3 Chan' num2str(chan_no(i)) '.tiff'];
                        saveas(gcf,filename);
                    else
                        if k==3
                            filename=['Tanuj/Mat_files/Result_figs/New (Acute_Neuromod_I)/T1/Cat_Block SC4 Chan' num2str(chan_no(i)) '.tiff'];
                            saveas(gcf,filename);
                        else
                            if k==4
                                filename=['Tanuj/Mat_files/Result_figs/New (Acute_Neuromod_I)/T1/Cat_Block SC5 Chan' num2str(chan_no(i)) '.tiff'];
                                saveas(gcf,filename);
                            end
                        end
                    end
...
Total_Cells= [length(not_eval)+length(chan_inc)+length(chan_dec)+length(chan_unchanged)];
Cells_Modulated= [length(chan_inc)+length(chan_dec)];
Cells_Modulated_Percentage=(Cells_Modulated/ (Total_Cells-length(not_eval)))*100;

Cells_Inc=[length(chan_inc)];
Cells_Inc_Percentage=(length(chan_inc)/(Total_Cells-length(not_eval)))*100;
Cells_Dec=[length(chan_dec)];
Cells_Dec_Percentage=(length(chan_dec)/ (Total_Cells-length(not_eval)))*100;
Cells_Unchanged=[length(chan_unchanged)];
Cells_Unchanged_Percentage=(length(chan_unchanged)/ (Total_Cells-length(not_eval)))*100;

Output.Total_Cells= [Total_Cells Cells_Modulated Cells_Inc Cells_Dec Cells_Unchanged  ]
Output.Cells_Modulated=[100 Cells_Modulated_Percentage Cells_Inc_Percentage  Cells_Dec_Percentage  Cells_Unchanged_Percentage]
Output.Inc_Cells=[chan_inc]'
Output.Per_Inc=[per_inc]'
Output.Dur_Inc=[dur_inc]'
Output.Dec_Cells=[chan_dec]'
Output.Per_Dec=[per_dec]'
Output.Dur_Dec=[dur_dec]'
