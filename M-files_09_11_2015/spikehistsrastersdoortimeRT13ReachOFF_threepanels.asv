...
%     hold off;
    ...
    plot ([(t(1):0.05:t(end))-t(1)],1*PEH_rate/y_max,'r','LineWidth',3);XLim([0 max_ax2]);YLim([0 1]);set(gca,'XTick',[0 f2 f2*2 f2*3 ]);set(gca,'XTickLabel',{'-2','-1','Reach OFF','1'})
    hold on;
    plot ([(t(1):0.05:t(end))-t(1)],1*PEH_rate2/y_max2,'k','LineWidth',3);XLim([0 max_ax2]);YLim([0 1]);set(gca,'XTick',[0 f2 f2*2 f2*3 ]);set(gca,'XTickLabel',{'-2','-1','Reach OFF','1'},'FontSize',14,'FontWeight','BOLD');set(gca,'YTick',[0 0.5 1 ]);set(gca,'YTickLabel',{'0','','1'},'FontSize',14,'FontWeight','BOLD');set(ax4, 'XGrid', 'on');set(ax4, 'YGrid', 'on');
    hold off;
    ...
    for l_incorr=1:length(incorr)
        ...
        subplot(3,1,2)
        plot(rastertics5{l_incorr},[length(corr) length(corr)+1]+l_incorr,'k','LineWidth',2);plot(doordelta_incorr(l_incorr),[length(corr) length(corr)+1]+l_incorr,'cx');XLim([0 max_ax2]);YLim([0 max_trials]);set(gca,'XTick',[0 1 2 3 ]);set(gca,'XTickLabel',{'-2','-1','Reach OFF','1'},'FontSize',14,'FontWeight','BOLD'); %set(gca,'YTickLabel','FontSize',14,'FontWeight','BOLD')%axis([0 2.4 (0+n4) (1+n4)]);%hold off;
        hold on
    end
    %xlabel('time (sec)')
    %     subplot(4,1,4)
...
%     
    ...
    plot(t,-mean_hold_x_corr,'r-','LineWidth',3)
    hold on;
    plot(t,(-mean_hold_x_incorr),'k-','LineWidth',3);XLim([(c3-60) (c3+36)]);YLim([(min(-mean_hold_x_corr)-5) (max(-mean_hold_x_corr)+5)]);set(gca,'XTick',[c3-60 c3-30 c3 c3+30]);set(gca,'XTickLabel',{'-2','-1','Reach OFF','1'}); set(gca,'YTick',[(min(-mean_hold_x_corr)-5) (((min(-mean_hold_x_corr)-5)+(max(-mean_hold_x_corr)+5))/2) (max(-mean_hold_x_corr)+5)]); set(gca,'YTickLabel',{'Initial','','Target'},'FontSize',14,'FontWeight','BOLD');xlabel ('time (sec)','FontSize',16,'FontWeight','BOLD');set(ax9, 'XGrid', 'on');set(ax9, 'YGrid', 'on');

    hold off;
    
        filename=['Mat_files/Result_figs/Block381_SC2_ReachOFF_SELECTDATAChan' num2str(chan_no) '.tiff'];
