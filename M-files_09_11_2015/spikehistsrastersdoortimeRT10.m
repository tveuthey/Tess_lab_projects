...
clear data_lfpfilt_incorr data_hil data_hil1 waves_corr waves_incorr waves_range2 rastertics2 doorind2 ind ind2
...
%Filter for LFPs with gamma band
...
for chan_no=[chan_no]
    for n=1:trials_NO
        temp_strobe(:,:,n)=strobe(Reach(n)-before_zero:Reach(n)+block_len); % plotting waves 4 s before and 1.2 after with door and grasp signal
  ...
  waves=Waves{chan_no,sortcode};
        ...
        if ~isempty(t2) && ~isempty(t3)
            ...
            doorind(:,:,n)=Reach(n)/Fs_wave- t(1);
                ...
                rastertics2{n}=[ts_norm];%; tstamps(range3)];
            
            doorind2(:,:,n)=Reach(n)/Fs_wave- t(1);
            end
        end
...
mean_data_hil_corr=mean(data_hil_corr,2);
mean_data_hil_incorr=mean(data_hil_incorr,2);
mean_data_lfpfilt_corr=mean(data_lfpfilt_corr,2);
mean_data_lfpfilt_incorr=mean(data_lfpfilt_incorr,2);
...
plot ([(t(1):0.050:t(end))-t(1)],1*PEH_rate/y_max,'r','LineWidth',2);XLim([0 5.]);set(gca,'XTick',[0 f f*2 f*3 f*4 f*5 f*6]);%set(gca,'XTickLabel',{'-1.2','-0.8','-0.4','0','0.4','0.8','1.2'})
hold on;
plot ([(t(1):0.050:t(end))-t(1)],1*PEH_rate2/y_max,'k','LineWidth',2);XLim([0 max_ax2]);set(gca,'XTick',[0 f f*2 f*3 f*4 f*5 f*6]);%set(gca,'XTickLabel',{'-1.2','-0.8','-0.4','0','0.4','0.8','1.2'})
hold off;


for n4=1:length(corr)
    [doordelta,ind]=sort(doorind,3);
    rastertics3{n4}=rastertics{ind(n4)};
    rastertics4{n4}=[rastertics3{n4};rastertics3{n4}];
    for n5=1:length(incorr)
    [doordelta2,ind2]=sort(doorind2,3);
    rastertics5{n5}=rastertics2{ind(n5)};
    rastertics6{n5}=[rastertics5{n5};rastertics5{n5}];
    subplot(4,1,4)
    if ismember(n4,corr)
        plot(rastertics4{n4},[0 1]+n4,'r','LineWidth',2);plot(doordelta(n4),[0 1]+n4,'gx');XLim([0 max_ax2]);%YLim([0 max_trials]);%set(gca,'XTick',[0 0.4 .8 1.2 1.6 2.0 2.4]);set(gca,'XTickLabel',{'-1.2','-0.8','-0.4','0','0.4','0.8','1.2'}) %axis([0 2.4 (0+n4) (1+n4)]);%hold off;
    else
        plot(rastertics6{n5},[length(corr) (length(corr)+1)]+n5,'k','LineWidth',2);plot(doordelta2(n5),[0 1]+n4,'gx');XLim([0 max_ax2]);%YLim([0 max_trials]);%set(gca,'XTick',[0 0.4 .8 1.2 1.6 2.0 2.4]);set(gca,'XTickLabel',{'-1.2','-0.8','-0.4','0','0.4','0.8','1.2'}) %axis([0 2.4 (0+n4) (1+n4)]);%hold off;
    end
    hold on
    end
end
