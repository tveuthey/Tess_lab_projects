...
% GET PAC
function [pac,troughPower,peakPower] = PAC_fn(data)
...
low_frequency = [0.5 4; 8 18]; % define theta and alpha bands
high_frequency = [8 18; 80 150]; % define gamma band

sampling_rate = 1017; % define sampling rate
...
%     
%     high_frequency_amplitude2 = eegfilt(raw_data(x, :), sampling_rate, high_frequency(3), []);
%     high_frequency_amplitude2 = eegfilt(high_frequency_amplitude2, sampling_rate, [], high_frequency(4));
%     high_frequency_amplitude2 = abs(hilbert(high_frequency_amplitude2));
% 
%     % Extract low frequency analytic phase
...
%     swsTroughEvents = find(low_frequency_phase < -(pi - 0.01));
%     swsPeakEvents = find((low_frequency_phase < (0 + 0.005)) & (low_frequency_phase > (0 - 0.005)));
%     swsTroughEvents = swsTroughEvents(swsTroughEvents > 20);
%     swsTroughEvents = swsTroughEvents(swsTroughEvents < (length(low_frequency_phase)-20));
%     swsPeakEvents = swsPeakEvents(swsPeakEvents > 20);
%     swsPeakEvents = swsPeakEvents(swsPeakEvents < (length(low_frequency_phase)-20));
...
% 
%     % Find peaks and troughs
%     spindleTroughEvents = find(low_frequency_phase < -(pi - 0.01));
%     spindlePeakEvents = find((low_frequency_phase < (0 + 0.005)) & (low_frequency_phase > (0 - 0.005)));
%     spindleTroughEvents = spindleTroughEvents(spindleTroughEvents > 20);
%     spindleTroughEvents = spindleTroughEvents(spindleTroughEvents < (length(low_frequency_phase)-20));
%     spindlePeakEvents = spindlePeakEvents(spindlePeakEvents > 20);
%     spindlePeakEvents = spindlePeakEvents(spindlePeakEvents < (length(low_frequency_phase)-20));
%     clear low_frequency_phase
% 
%     troughPower = [];
% peakPower=[];
...
% %     troughPower(x, 1) = mean(ERP(high_frequency_amplitude, sampling_rate, swsTroughEvents, window_size(1), window_size(1)));
% %     troughPower(x, 2) = mean(ERP(high_frequency_amplitude, sampling_rate, spindleTroughEvents, window_size(2), window_size(2)));
% %     troughPower(x, 3) = mean(ERP(high_frequency_amplitude2, sampling_rate, swsTroughEvents, window_size(1), window_size(1)));
% %     troughPower(x, 4) = mean(ERP(high_frequency_amplitude2, sampling_rate, spindleTroughEvents, window_size(2), window_size(2)));
% %     peakPower(x, 1) = mean(ERP(high_frequency_amplitude, sampling_rate, swsPeakEvents, window_size(1), window_size(1)));
% %     peakPower(x, 2) = mean(ERP(high_frequency_amplitude, sampling_rate, spindlePeakEvents, window_size(2), window_size(2)));
% %     peakPower(x, 3) = mean(ERP(high_frequency_amplitude2, sampling_rate, swsPeakEvents, window_size(1), window_size(1)));
% %     peakPower(x, 4) = mean(ERP(high_frequency_amplitude2, sampling_rate, spindlePeakEvents, window_size(2), window_size(2)));
% %         
