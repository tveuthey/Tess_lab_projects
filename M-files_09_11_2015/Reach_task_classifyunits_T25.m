function classify_units(units,Reach,TimeStamps,
...
before_zero=1017; % looking 1.0 sec before and 1.0 sec after reach start 1017 samples/sec

sortcode=2; %use 1 for MU activity, and subsequently 2/3/4 depending on sorted unit yield per channel
bin_size = 100;

for chan_no =chan_nos(i)
    peak(1,i)= chan_no;
    for n=1:length(Reach);
        t=[Reach(n)-trl_len:Reach(n)+trl_len]/Fs_lfp;
        ...
        if(~isempty (trial1) && ~isempty(trial2))
            ...
            if ~isempty(find(binned2))
                hold on
                plot(find(binned2)/Fs_lfp,n,'k.','LineWidth',2); YLim([1 trials_NO]); XLim([0 (length(binned3)/Fs_lfp)])
                %plot(b_gauss,ii)
                %pause;
            end
            %         pause;
            end
        %subplot(212); plot(4,1:length(trials_NO),'-','LineWidth',3);
    end
t = strcat('Unit ',int2str(chan_no),int2str(sortcode));
title(t);
% pause;
hold off;

mfr = mean(bin_rate_gauss);
mfr_1 = mean(bin_rate_gauss(:,1:60));
mfr_2 = mean(bin_rate_gauss(:,61:end));
...
%saveas(gcf,output,'tiff')
 saveas(gcf,strcat('Task_Block2_Unit',int2str(chan_no),' ',int2str(sortcode)),'tiff')
%
% close all;
hold off;
pause;
