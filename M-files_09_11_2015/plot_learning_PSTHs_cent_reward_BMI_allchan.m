function [Output]=plot_learning_PSTHs_cent_reward_BMI_allchan(TimeStamps, chan_no,data,wave,start_trial, start_pulse2, Fs_lfp,Waves,N,sc,bin_size,space)
% close all;
length_to_analyze=length(data);
block=1:length_to_analyze;

before_zero=round(8*Fs_lfp);
...
%             disp(num2str(n))
       ...
       PEH_rate1(i,:)=mean(bin_rate_s(:,1:15),2);
       PEH_rate1(i,:)=PEH_rate1(i,:)/max(PEH_rate1(i,:))
        if i==1
            figure(1)
        plot (bin_vector,smooth(PEH_rate1(i,:),25),'r','LineWidth',3); hold on
        else
            figure(1)
            plot (bin_vector,smooth(PEH_rate1(i,:),'sgolay'),'c','LineWidth',2);
            hold on
        end
        hold on
...
% PEH_rate3=mean(bin_rate_s(:,81:104),2);
ymax= max(PEH_rate1);
y_max=max(ymax);
% figure
...
%     legend('First 1/3rd trials','Second 1/3rd trials','Last 1/3rd trials','Location','NorthWest')
               filename=['C:\Users\Ganguly Lab\Documents\MATLAB\Tanuj\Mat_files\Result_figs\BMI\S32\5_29_13\Task Every Third Trial PSTH_cent_reward Ch' num2str(chan_no) '.tiff'];
