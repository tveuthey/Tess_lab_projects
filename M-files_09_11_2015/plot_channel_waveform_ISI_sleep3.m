...
function [Change_firing]=plot_channel_waveform_ISI_sleep3(Waves1,Waves2,TimeStamps1,TimeStamps2,N,units,sleep_ind_pre, sleep_ind_post,Fs_lfp)
close all
for i=1:length(units)
    ...
    T1=TimeStamps1{chan_no,N};
    T2=TimeStamps2{chan_no,N};
    l1=(sleep_ind_pre(1,1):sleep_ind_pre(end,end));
    l2=(sleep_ind_post(1,1):sleep_ind_post(end,end));
    ...
    %if N==2
        filename= ['C:\Users\dhaks_000\Documents\MATLAB\Dhakshin\Mat_files\raw_data\Reach\T12_allblocks\censored\Sleep3nsSleep4ns\Waveform SC' int2str(N) 'Ch' num2str(chan_no) '.tiff'];
        set(gcf, 'Position', get(0,'Screensize'));
        saveas(gcf,filename);
    %elseif N ==3
