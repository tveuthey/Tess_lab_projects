function plot_raster_peth_alltrials_T39(trial_start,trial_marker,TimeStamps,len_bef,len_aft,bin,dir,type)
...
for i = 1:length(TimeStamps)
    ...
    if (max(max(peth(count).binearly))>1 && max(max(peth(count).binlate))>1)
        ...
        t2 = strcat(dir,'PETH Unit',num2str(i),'SC',num2str(sc),'Centered to ',type,'alltrials');
        %pause;
        saveas(h,t2,'tiff')
        close all;
