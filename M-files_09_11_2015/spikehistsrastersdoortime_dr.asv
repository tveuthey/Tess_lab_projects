...
block_len=2034;
before_zero=2034; % looking 2.0 sec before and 2.0 sec after door open time at 1017 samples/sec
figure;
chan_no_forchew=23;
% chan_nos=[9 11 12 14 16 27 29];
...
%         disp(n)
        ...
        plot(f(35:45),sm_power(35:45))
        hold on;
        hold_val(n)=mean(sm_power(35:45));
...
corr=find(label==2);
incorr=find(label==1);
corr2=[find(hold_val>0.3)]';
incorr2=[find(hold_val<0.3)]';
...
for n4=1:trials_NO-1
    ...
    if ~isempty(rastertics2{n4}) 
        rastertics3{n4}=[rastertics2{n4};rastertics2{n4}];
        subplot(4,1,4)
        if ismember(n4,corr)
            plot(rastertics3{n4},[0 1]+n4,'r','LineWidth',2);plot(doordelta(n4),[0 1]+n4,'gx');XLim([0 max_ax2]);YLim([0 max_trials]);%set(gca,'XTick',[0 0.4 .8 1.2 1.6 2.0 2.4]);set(gca,'XTickLabel',{'-1.2','-0.8','-0.4','0','0.4','0.8','1.2'}) %axis([0 2.4 (0+n4) (1+n4)]);%hold off;
        else
            plot(rastertics3{n4},[0 1]+n4,'k','LineWidth',2);plot(doordelta(n4),[0 1]+n4,'gx');XLim([0 max_ax2]);YLim([0 max_trials]);%set(gca,'XTick',[0 0.4 .8 1.2 1.6 2.0 2.4]);set(gca,'XTickLabel',{'-1.2','-0.8','-0.4','0','0.4','0.8','1.2'}) %axis([0 2.4 (0+n4) (1+n4)]);%hold off;
        end
    hold on
    end
end
...
%cd ('C:\Users\dhaks_000\Documents\MATLAB\Dhakshin\Mat_files\raw_data\S34\Task_Block1\')
output = strcat('C:\Users\dhaks_000\Documents\MATLAB\Dhakshin\Mat_files\raw_data\Reach\S34\Task_Block2\', int2str(chan_no));
