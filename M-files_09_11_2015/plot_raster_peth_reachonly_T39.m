function plot_raster_peth_reachonly_T39(trial_start,trial_marker,TimeStamps,len_bef,len_aft,bin,dir,type)
...
for i = 1:length(TimeStamps)
    ...
    peth(count).binearly = nansum(ts_bin(1:cnte,:));
    peth(count).binlate = nansum(ts_bin(end-cntl+1:end,:));
    if (max(max(peth(count).binearly))>1 && max(max(peth(count).binlate))>1)
        ...
        t2 = strcat(dir,'PETH Unit',num2str(i),'SC',num2str(sc),'Centered to ',type,'reachtrials');
        %       pause;
        saveas(h,t2,'tiff')
        close all;
...
for l = 1:length(trp)
    if trp(l)~=0
        ...
        h(c) = plot(bin2,peth(l).fit.mean, 'Color', colors(c, :),...
            'DisplayName',sprintf('%d,%d',peth(l).un(1),peth(1).un));
...
for l = 1:length(trn)
    if trn(l)~=0
        ...
        c = c+1;
        h(c) = plot(bin2,peth(l).fit.mean, 'Color', colors(c, :),...
            'DisplayName',sprintf('%d,%d',peth(l).un(1),peth(l).un));
        tm(:,c)=peth(l).fit.mean;
    end
end
yl = max(max(tm))+2;
...
close all;
%pause;
savefile = [dir,'peth_reachtrialsonly_centeredto',type,'.mat'];
