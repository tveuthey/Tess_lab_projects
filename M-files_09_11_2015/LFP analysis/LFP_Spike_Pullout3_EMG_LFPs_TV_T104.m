...
% CHANGE THE CHANNELS THAT WERE EMG (Line 139)
% change selected_data portion at the bottom to make a structure that only
% has the data you want

% Block-4 and Block-41 had to be extracted using TDT2mat. Only Block-4 had to have
% '-v7.3' appended to end because the file size was too big. See saving
% code at the end.


%% CODE FROM HERE DOWN DOES NOT NEED TO BE CHANGED (ok some at the end to
% % select which channels belong to which brain regions)
...
dec_factor=1;
Fs_sEMG=Fs_wave/dec_factor
for chan_no=[3 7]; %% need to change the channels that were EMG
    disp(chan_no)
    get_chan=chan(chan_no);
    TTX.SetGlobalV('Channel',get_chan);
...
% save (['data_block_T111_3'])

    for good_channels=[29,25,31,27,13,11,30,28,2,4,6,8,7,3];
        selected_data(good_channels).ch=good_channels;
        selected_data(good_channels).data=data(good_channels,:);
    end
    for M1=[25,27,29,31];
        selected_data(M1).location='M1';
    end
    for M2=[13,11,30,28];
        selected_data(M2).location='M2';
    end
    for Ox=[2,4,6,8];
        selected_data(Ox).location='Ox';
    end
    for EMG=[3,7];
        selected_data(EMG).location='EMG';
    end

save([name])
end

%% Block-4 and Block-41 using TDT2mat
%% Block-4
clear all

extracted=TDT2mat('T104','Block-4')
save('data_block_T104_4_TDT2mat_all_results', '-v7.3')
data=extracted.streams.LFPs.data;

for good_channels=[29,25,31,27,13,11,30,28,2,4,6,8,7,3];
        selected_data(good_channels).ch=good_channels;
        selected_data(good_channels).data=extracted.streams.LFPs.data(good_channels,:);
    end
    for M1=[25,27,29,31];
        selected_data(M1).location='M1';
    end
...
save('data_block_T104_4', 'selected_data')


%% Block-41
clear all

extracted=TDT2mat('T104','Block-41')
% save('data_block_T104_41_TDT2mat_all_results', '-v7.3')
save('data_block_T104_41_TDT2mat_all_results')
data=extracted.streams.LFPs.data;
...
save('data_block_T104_41', 'selected_data')
