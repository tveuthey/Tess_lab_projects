function [Activities, Patterns, eigenvalues, lambda_max]=Reach_task_cellassemblyb_T39(TimeStamps,trial_marker,Fs_lfp,bin,units,percblock,len_bef,len_aft,markertype)
...
%              end
        ...
        if ~isempty(dN_cell_all)
            ...
            Task_bin_all(count_all,:) = zeros(1,1:length(Task_bin_all));
            Task_bin_end(count_all,:) = zeros(1,1:length(Task_bin_end));
        end
        %end
    end
